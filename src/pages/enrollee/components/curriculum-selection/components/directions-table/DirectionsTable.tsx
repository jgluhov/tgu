...
const DirectionsTable = (props: IEnrolleeScheduleProps) => {
  ...
  return (
    <table className={clsx(commonStyles.curriculumSelection, styles.directionsTable)} {...getTableProps()}>
      <thead>
        { headerGroups.map((headerGroup, rowIndx) => (
          <tr className={commonStyles.curriculumSelectionHeaderRow} {...headerGroup.getHeaderGroupProps()}>
            { headerGroup.headers.map(column => (
                <th className={clsx(
                  commonStyles.curriculumSelectionHeader,
        ...
        { rows.map((row, rowIndx) => {
          ...
          return (
            <tr className={clsx(rowIndx === 0 && styles.directionsTableHeaderFirst)} {...row.getRowProps()}>
              { row.cells.map(cell => (
                  <td className={clsx(
                    styles.directionsTableData,
                    row.original.isPreferred && styles.directionsTableDataPreferred,
                    rowIndx === 0 && styles.directionsTableDataFirst)
                  } {...cell.getCellProps()}>
                    { cell.render('Cell')}
                  </td>
                )
